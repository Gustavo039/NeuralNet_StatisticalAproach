...
* $\mathbf{Y}$ é um vetor $n \times 1$ contendo os valores da variável resposta

* $\mathbf{X}$ é um matriz $n \times p$ contendo os valores das variáveis explicativas (com $n$ observações e $p$ variáveis)

* $\beta$ é um vetor $p \times 1$ de coeficientes

* $\boldsymbol{\epsilon}$ é um vetor $n \times 1$ de efeitos aleatórios

Como descrito anteriormente, visualmente, a regressão linear busca traçar uma reta entre os dados de treinamento e para isso o método utilizado para encontrar essa é o chamado: **Método dos Mínimos Quadrados**

Esse método tem como objetivo minimizar a soma quadrado dos resíduos, dado por:

$$min \ \ || \mathbf{Y} -  \hat{\mathbf{Y}}||^2$$

onde $\hat{\mathbf{Y}} =  \mathbf{X} \hat{\boldsymbol{\beta}}$

A solução desse problema possui forma fechada, dada por: 

$$\hat{\boldsymbol{\beta}} = (X^TX)^{-1}X^TY$$

Para que o método funcionar, algumas condições em relação a matrix $\mathbf{X}$ devem ser respeitadas

* Linearmente Independentes
  - Suas colunas devem ser linarmente independentes. Isso é, as variávies utilizadas no modelo devem ser linearmente independentes. Por exemplo, um modelo que utiliza a idade de um individuo e sua data de nascimento, que são variáveis linearmente dependentes possui altas chances de não convergir 

* Posto Completo (n > p)
  - Os números de linhas da matriz deve ser maior que o número de colunas. Isso é, o número de observações de treinamento do modelo deve ser maior que o número de variáveis. Se p > n, a matrix $(X^TX)$ se torna não invertivel, tornando os calculos de $\beta$ impossíveis
